...
export class ConsoleApp {
    ...
    constructor(handleOutput) {
        ...
        this.domainSuffix = "xyz";
        this.MOVIE_BASE_URL = `https://vidsrc.${this.domainSuffix}/embed/movie?tmdb=`;
        this.TV_BASE_URL = `https://vidsrc.${this.MOVIE_BASE_URL}/embed/tv?tmdb=`;
    }
    async handleCommand(input) {
        const trimmedInput = input.trim().toLowerCase();
        if (!trimmedInput || trimmedInput === 'exit')
            return;
    ...
    async selectByNameOrIndex(argument) {
        try {
            let success = false;
            ...
            switch (parts.length) {
                case 1: // p {episode number or movie/show name}
                    ...
                    if (!isNaN(num) && this.currentSeason) {
                        this.playEpisode(MovieDbService.getEpisodeUrl(this.currentMediaResult.id, this.currentSeason.season_number, num));
    ...
    playNextEpisode() {
        ...
        this.playEpisode(MovieDbService.getEpisodeUrl(this.currentMediaResult.id, this.currentSeason.season_number, this.currentEpisodeIndex + 1));
