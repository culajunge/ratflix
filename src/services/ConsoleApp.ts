...
export class ConsoleApp {
    ...
    private async downloadMedia(argument: string): Promise<void> {
        ...
        if(!this.currentMediaResult && argument.length > 0){
            this.currentSearchResult = await MovieDbService.searchMulti(argument, true);
            this.currentMediaResult = this.currentSearchResult.results[0];
        }
        else if(!this.currentMediaResult) {
            this.handleOutput("No media selected. Please select a media first.");
            return;
        }

        try {
            const videoUrl = this.currentMediaResult.media_type === "movie"
                ? MovieDbService.getMovieUrl(this.currentMediaResult.id)
                : MovieDbService.getEpisodeUrl(
                    this.currentMediaResult.id,
                    this.currentSeason!.season_number,
                    this.currentEpisodeIndex
                );

            // Launch browser and navigate to video
            ...
            page.on('request', request => {
                if (request.url().includes('.m3u8')) {
                    this.handleOutput(`Found stream URL: ${request.url()}`);
                    this.initiateDownload(request.url());
                }
                request.continue();
            });

            await page.goto(videoUrl);
            await page.click('.play-button'); // Click play to start stream

            // Wait for stream to be found
            await new Promise(resolve => setTimeout(resolve, 5000));
            await browser.close();

        } catch (error) {
            this.handleOutput(`Download failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }

    private async initiateDownload(streamUrl: string): Promise<void> {
        const filename = `${this.currentMediaResult!.title || this.currentMediaResult!.name}.mp4`;

        const ffmpeg = new FFmpeg();
        await ffmpeg.load();

        this.handleOutput(`Starting download of ${filename}...`);
        await ffmpeg.exec(['-i', streamUrl, filename]);
